...
def update_status_endpoint(payment_id:UUID,payload:PaymentUpdateSchema,db:Session=Depends(get_db)):
...
def get_list_of_payments_endpoint(db:Session=Depends(get_db),skip:int=Query(0,ge=0),limit:int=Query(100,ge=1,le=100)):

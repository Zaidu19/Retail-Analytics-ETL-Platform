...
def create_inventory_logs_endpoint(payload:InventoryLogCreateSchema,db:Session=Depends(get_db)):
...
def get_inventory_log_by_id_endpoint(inventory_log_id:UUID,db:Session=Depends(get_db)):
...
def get_list_inventory_logs_endpoint(db:Session=Depends(get_db),skip:int= Query(0,ge=0),limit:int=Query(100,ge=1,le=100)):
